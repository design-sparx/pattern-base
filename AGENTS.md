...
- Public shell (header/nav/footer) lives in `apps/docs/src/components/layout/public-shell/`: `PublicShellLayout` (server component) wraps `<main id="main-content">` in a `min-h-screen` flex column; `PublicHeader` (sticky) → `MainNav` (shadcn NavigationMenu) / `MobileNav` (vaul Drawer); `PublicFooter` (multi-column). Nav/footer data comes from `apps/docs/src/lib/public-shell.ts` (`getNavCategories()` reads `@/data/patterns` with live counts — keep static text like "54 AI-UX patterns" in sync if counts change). Use the `app-container` utility (defined in `globals.css`) instead of hardcoding `max-w-7xl px-4` to keep header/body/footer edges aligned.

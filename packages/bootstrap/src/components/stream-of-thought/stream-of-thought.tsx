  ...
  return (
    <Stack gap={2}>
      <div className="d-flex align-items-center gap-2">
        ...
        {isStreaming && <Spinner animation="border" size="sm" />}
      ...
      {collapsible ? (
        ...
        >
          {steps.map((step, index) => {
            const config = STEP_CONFIG[step.type] ?? { icon: '\u2022', color: '#6b7280' };
            return (
              <Accordion.Item key={step.id} eventKey={step.id}>
                <Accordion.Header>
                  <div className="d-flex align-items-center gap-2 flex-grow-1">
                    ...
                    <span className="text-muted small text-truncate" style={{ maxWidth: 300 }}>
          ...
          {steps.map((step, index) => {
            ...
            return (
              <div key={step.id} className="border rounded p-2">
                ...
                <p className="mb-0 small">{step.content}</p>
              </div>
            );
          })}
        </Stack>

        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <Space direction="vertical" size={2}>
            <Space>
              ...
              <Tag color={getColor(relevance)}>{getLabel(relevance)} Relevance</Tag>
            </Space>
            {url && (
              <a href={url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12 }}>
                <LinkOutlined /> {url.length > 60 ? url.substring(0, 60) + '...' : url}
              </a>
            )}
          </Space>
  ...
  return (
    <Space direction="vertical" style={{ width: '100%' }}>
      <Space>
        ...
        <Badge count={citations.length} style={{ backgroundColor: '#6b7280' }} />
      </Space>
